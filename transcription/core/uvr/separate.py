...
# This code is modified from https://github.com/seanghay/uvr/blob/main/separate.py
...
class  _audio_pre_():
    ...
    def predict(self, audio, target_sr=44100):
        ...
        wavfile.write('instrument_test.wav', self.mp.param['sr'], (np.array(wav_instrument)*32768).astype("int16"))
        ...
        print ('vocals done')
        wavfile.write('vocal_test.wav', self.mp.param['sr'], (np.array(wav_vocals)*32768).astype("int16"))

        wav_instrument = wav_instrument.astype(np.float32)
        wav_vocals = wav_vocals.astype(np.float32)
        return wav_vocals, wav_instrument
